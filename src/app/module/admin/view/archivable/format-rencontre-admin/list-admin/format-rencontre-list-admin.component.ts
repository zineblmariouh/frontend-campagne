...
public async archiverFormatRencontre(formatRencontre:FormatRencontreVo){
const isPermistted = await this.roleService.isPermitted('FormatRencontre', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Format rencontre) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.formatRencontreService.archiver(formatRencontre).subscribe(status=>{
const myIndex = this.formatRencontres.indexOf(formatRencontre);
this.formatRencontres[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Format rencontre archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverFormatRencontre(formatRencontre:FormatRencontreVo){
const isPermistted = await this.roleService.isPermitted('FormatRencontre', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Format rencontre) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.formatRencontreService.desarchiver(formatRencontre).subscribe(status=>{
const myIndex = this.formatRencontres.indexOf(formatRencontre);
this.formatRencontres[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Format rencontre désarchivé',
life: 3000
});
},error=>console.log(error))
 }
