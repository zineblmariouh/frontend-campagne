...
public async archiverInstrumentIrd(instrumentIrd:InstrumentIrdVo){
const isPermistted = await this.roleService.isPermitted('InstrumentIrd', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Instrument ird) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.instrumentIrdService.archiver(instrumentIrd).subscribe(status=>{
const myIndex = this.instrumentIrds.indexOf(instrumentIrd);
this.instrumentIrds[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Instrument ird archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverInstrumentIrd(instrumentIrd:InstrumentIrdVo){
const isPermistted = await this.roleService.isPermitted('InstrumentIrd', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Instrument ird) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.instrumentIrdService.desarchiver(instrumentIrd).subscribe(status=>{
const myIndex = this.instrumentIrds.indexOf(instrumentIrd);
this.instrumentIrds[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Instrument ird désarchivé',
life: 3000
});
},error=>console.log(error))
 }
