...
public async archiverPubliqueProfessionel(publiqueProfessionel:PubliqueProfessionelVo){
const isPermistted = await this.roleService.isPermitted('PubliqueProfessionel', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Publique professionel) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.publiqueProfessionelService.archiver(publiqueProfessionel).subscribe(status=>{
const myIndex = this.publiqueProfessionels.indexOf(publiqueProfessionel);
this.publiqueProfessionels[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Publique professionel archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverPubliqueProfessionel(publiqueProfessionel:PubliqueProfessionelVo){
const isPermistted = await this.roleService.isPermitted('PubliqueProfessionel', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Publique professionel) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.publiqueProfessionelService.desarchiver(publiqueProfessionel).subscribe(status=>{
const myIndex = this.publiqueProfessionels.indexOf(publiqueProfessionel);
this.publiqueProfessionels[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Publique professionel désarchivé',
life: 3000
});
},error=>console.log(error))
 }
