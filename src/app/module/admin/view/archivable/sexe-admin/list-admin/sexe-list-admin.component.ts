         ...
         if(isPermistted){
          this.sexeService.findByIdWithAssociatedList(sexe).subscribe(res => {
           this.selectedSexe = res;
            this.selectedSexe.dateArchivage = new Date(sexe.dateArchivage);
            this.selectedSexe.dateCreation = new Date(sexe.dateCreation);
...
public async archiverSexe(sexe:SexeVo){
const isPermistted = await this.roleService.isPermitted('Sexe', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Sexe) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.sexeService.archiver(sexe).subscribe(status=>{
const myIndex = this.sexes.indexOf(sexe);
this.sexes[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Sexe archivé',
...
public async desarchiverSexe(sexe:SexeVo){
const isPermistted = await this.roleService.isPermitted('Sexe', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Sexe) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.sexeService.desarchiver(sexe).subscribe(status=>{
const myIndex = this.sexes.indexOf(sexe);
this.sexes[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Sexe désarchivé',
life: 3000
});
},error=>console.log(error))
 }
  ...
  initExport(): void {
    ...
    return {
      ...
      this.criteriaData = [{
            ...
            'Admin': this.searchSexe.admin ? (this.searchSexe.admin ? environment.trueValue : environment.falseValue) : environment.emptyForExport ,
            'Visible': this.searchSexe.visible ? (this.searchSexe.visible ? environment.trueValue : environment.falseValue) : environment.emptyForExport ,
            'Username': this.searchSexe.username ? this.searchSexe.username : environment.emptyForExport ,
     }];
