...
public async archiverStatusCursus(statusCursus:StatusCursusVo){
const isPermistted = await this.roleService.isPermitted('StatusCursus', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Status cursus) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.statusCursusService.archiver(statusCursus).subscribe(status=>{
const myIndex = this.statusCursuss.indexOf(statusCursus);
this.statusCursuss[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Status cursus archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverStatusCursus(statusCursus:StatusCursusVo){
const isPermistted = await this.roleService.isPermitted('StatusCursus', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Status cursus) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.statusCursusService.desarchiver(statusCursus).subscribe(status=>{
const myIndex = this.statusCursuss.indexOf(statusCursus);
this.statusCursuss[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Status cursus désarchivé',
life: 3000
});
},error=>console.log(error))
 }
