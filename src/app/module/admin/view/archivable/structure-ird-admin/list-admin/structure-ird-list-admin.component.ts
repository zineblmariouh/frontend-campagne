...
public async archiverStructureIrd(structureIrd:StructureIrdVo){
const isPermistted = await this.roleService.isPermitted('StructureIrd', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Structure ird) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.structureIrdService.archiver(structureIrd).subscribe(status=>{
const myIndex = this.structureIrds.indexOf(structureIrd);
this.structureIrds[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Structure ird archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverStructureIrd(structureIrd:StructureIrdVo){
const isPermistted = await this.roleService.isPermitted('StructureIrd', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Structure ird) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.structureIrdService.desarchiver(structureIrd).subscribe(status=>{
const myIndex = this.structureIrds.indexOf(structureIrd);
this.structureIrds[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Structure ird désarchivé',
life: 3000
});
},error=>console.log(error))
 }
