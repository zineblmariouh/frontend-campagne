...
public async archiverTemplateCloture(templateCloture:TemplateClotureVo){
const isPermistted = await this.roleService.isPermitted('TemplateCloture', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Template cloture) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.templateClotureService.archiver(templateCloture).subscribe(status=>{
const myIndex = this.templateClotures.indexOf(templateCloture);
this.templateClotures[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Template cloture archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverTemplateCloture(templateCloture:TemplateClotureVo){
const isPermistted = await this.roleService.isPermitted('TemplateCloture', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Template cloture) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.templateClotureService.desarchiver(templateCloture).subscribe(status=>{
const myIndex = this.templateClotures.indexOf(templateCloture);
this.templateClotures[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Template cloture désarchivé',
life: 3000
});
},error=>console.log(error))
 }
