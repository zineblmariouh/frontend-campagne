...
public async archiverTemplateRelance(templateRelance:TemplateRelanceVo){
const isPermistted = await this.roleService.isPermitted('TemplateRelance', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Template relance) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.templateRelanceService.archiver(templateRelance).subscribe(status=>{
const myIndex = this.templateRelances.indexOf(templateRelance);
this.templateRelances[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Template relance archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverTemplateRelance(templateRelance:TemplateRelanceVo){
const isPermistted = await this.roleService.isPermitted('TemplateRelance', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Template relance) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.templateRelanceService.desarchiver(templateRelance).subscribe(status=>{
const myIndex = this.templateRelances.indexOf(templateRelance);
this.templateRelances[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Template relance désarchivé',
life: 3000
});
},error=>console.log(error))
 }
