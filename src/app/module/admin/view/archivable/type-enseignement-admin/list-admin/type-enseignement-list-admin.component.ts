...
@Component({
  selector: 'app-type-enseignement-list-admin',
  templateUrl: './type-enseignement-list-admin.component.html',
  styleUrls: ['./type-enseignement-list-admin.component.css']
})
export class TypeEnseignementListAdminComponent implements OnInit {
   // declarations
...
public async archiverTypeEnseignement(typeEnseignement:TypeEnseignementVo){
const isPermistted = await this.roleService.isPermitted('TypeEnseignement', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Type enseignement) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.typeEnseignementService.archiver(typeEnseignement).subscribe(status=>{
const myIndex = this.typeEnseignements.indexOf(typeEnseignement);
this.typeEnseignements[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Type enseignement archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverTypeEnseignement(typeEnseignement:TypeEnseignementVo){
const isPermistted = await this.roleService.isPermitted('TypeEnseignement', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Type enseignement) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.typeEnseignementService.desarchiver(typeEnseignement).subscribe(status=>{
const myIndex = this.typeEnseignements.indexOf(typeEnseignement);
this.typeEnseignements[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Type enseignement désarchivé',
life: 3000
});
},error=>console.log(error))
 }
