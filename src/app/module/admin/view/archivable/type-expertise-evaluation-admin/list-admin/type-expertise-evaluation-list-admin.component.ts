...
@Component({
  selector: 'app-type-expertise-evaluation-list-admin',
  templateUrl: './type-expertise-evaluation-list-admin.component.html',
  styleUrls: ['./type-expertise-evaluation-list-admin.component.css']
})
export class TypeExpertiseEvaluationListAdminComponent implements OnInit {
   // declarations
...
public async archiverTypeExpertiseEvaluation(typeExpertiseEvaluation:TypeExpertiseEvaluationVo){
const isPermistted = await this.roleService.isPermitted('TypeExpertiseEvaluation', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Type expertise evaluation) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.typeExpertiseEvaluationService.archiver(typeExpertiseEvaluation).subscribe(status=>{
const myIndex = this.typeExpertiseEvaluations.indexOf(typeExpertiseEvaluation);
this.typeExpertiseEvaluations[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Type expertise evaluation archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverTypeExpertiseEvaluation(typeExpertiseEvaluation:TypeExpertiseEvaluationVo){
const isPermistted = await this.roleService.isPermitted('TypeExpertiseEvaluation', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Type expertise evaluation) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.typeExpertiseEvaluationService.desarchiver(typeExpertiseEvaluation).subscribe(status=>{
const myIndex = this.typeExpertiseEvaluations.indexOf(typeExpertiseEvaluation);
this.typeExpertiseEvaluations[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Type expertise evaluation désarchivé',
life: 3000
});
},error=>console.log(error))
 }
