...
@Component({
  selector: 'app-type-savoir-list-admin',
  templateUrl: './type-savoir-list-admin.component.html',
  styleUrls: ['./type-savoir-list-admin.component.css']
})
export class TypeSavoirListAdminComponent implements OnInit {
   // declarations
...
public async archiverTypeSavoir(typeSavoir:TypeSavoirVo){
const isPermistted = await this.roleService.isPermitted('TypeSavoir', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Type savoir) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.typeSavoirService.archiver(typeSavoir).subscribe(status=>{
const myIndex = this.typeSavoirs.indexOf(typeSavoir);
this.typeSavoirs[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Type savoir archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverTypeSavoir(typeSavoir:TypeSavoirVo){
const isPermistted = await this.roleService.isPermitted('TypeSavoir', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Type savoir) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.typeSavoirService.desarchiver(typeSavoir).subscribe(status=>{
const myIndex = this.typeSavoirs.indexOf(typeSavoir);
this.typeSavoirs[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Type savoir désarchivé',
life: 3000
});
},error=>console.log(error))
 }
