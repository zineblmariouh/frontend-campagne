...
public async archiverZoneGeographique(zoneGeographique:ZoneGeographiqueVo){
const isPermistted = await this.roleService.isPermitted('ZoneGeographique', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Zone geographique) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.zoneGeographiqueService.archiver(zoneGeographique).subscribe(status=>{
const myIndex = this.zoneGeographiques.indexOf(zoneGeographique);
this.zoneGeographiques[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Zone geographique archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverZoneGeographique(zoneGeographique:ZoneGeographiqueVo){
const isPermistted = await this.roleService.isPermitted('ZoneGeographique', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Zone geographique) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.zoneGeographiqueService.desarchiver(zoneGeographique).subscribe(status=>{
const myIndex = this.zoneGeographiques.indexOf(zoneGeographique);
this.zoneGeographiques[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Zone geographique désarchivé',
life: 3000
});
},error=>console.log(error))
 }
