...
@Component({
  selector: 'app-encadrement-list-admin',
  templateUrl: './encadrement-list-admin.component.html',
  styleUrls: ['./encadrement-list-admin.component.css']
})
export class EncadrementListAdminComponent implements OnInit {
   // declarations
    ...
    public async loadEncadrements(){
       const chercheur = this.authService.authenticatedUserByAdmin();
        ...
        if (chercheur !== null){
            ...
            isPermistted ? this.encadrementService.findByChercheurId(chercheur.id).subscribe(encadrements => this.encadrements = encadrements,error => console.log(error))
         ...
         if(isPermistted){
          this.encadrementService.findByIdWithAssociatedList(encadrement).subscribe(res => {
           this.selectedEncadrement = res;
            this.editEncadrementDialog = true;
          });
        }else{
